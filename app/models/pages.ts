...
export function createPage(req: Request, res: Response, next: NextFunction) {
	...
	db.none(
		...
		.catch(function catchError(err: Error) {
			return next(err);
		});
}
...
export function readPage(req: Request, res: Response, next: NextFunction) {
	 ...
	 */
	if (!id) {
		const error = new Error('httpStatusCodetatus code 400: Missing id');
		return next(error);
	}
 ...
 * @param {NextFunction} next
 */
export function readAllPages(req: Request, res: Response, next: NextFunction) {
	/**
...
export function incorrectIDMiddleware(
	req: Request,
	res: Response,
	next: NextFunction,
	id: string,
